...
<div class="container" style="margin-top: 10px;">
    <div class="alert alert-light" role="alert" style="margin-top: 20px; border-color: black; color: black;">
        Modifica campagna # {{ $campaigns->id }} {{ $campaigns->name }}
    </div>
    ...
    @if (session('overlapFound') || session('warning'))
        <div class="alert alert-danger" role="alert" style="width: fit-content; margin: 0 auto; margin-bottom: 15px;">
            ...
            <span>
                @if (session('warning'))
                    Non puoi inserire una campagna uguale
                @endif
                @if (session('overlapFound'))
                    ATTENZIONE! Esistono le seguenti campagne in conflitto con la modifica/inserimento corrente. Verificare.<br>
        ...
        <div class="col" action="">
            ...
            @csrf
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="nomeCampagna" name="nomeCampagna" value="{{ $campaigns->name }}" oninput="checkEmptiness()">
                </div>
                <div class="mb-3">
            ...
            @csrf
                ...
                <div class="mb-3">
                    <label>Messaggio</label>
                    <textarea class="form-control" id="testo" name="testo" rows="3">{{ $campaigns->message }}</textarea>
                </div>
                <div class="mb-3">
                    <label id="allCustomerLabel">Tutti i chiamanti</label>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="allCustomer" id="allCustomer1" value="1" onchange="checkEmptiness()" {{ $campaigns->allCustomers == 1 ? 'checked' : '' }}>
                        <label class="form-check-label" for="allCustomer1">
                            Sì
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="allCustomer" id="allCustomer2" value="0" onchange="checkEmptiness()" {{ $campaigns->allCustomers == 0 ? 'checked' : '' }}>
                        <label class="form-check-label" for="allCustomer2">
                            No
                        </label>
                    </div>
                    ...
                    <label id="abbattimentoLabel">Abbattimento</label>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="abbattimento" id="abbattimento1" value="1" onchange="checkEmptiness()" {{ $campaigns->dropCall == 1 ? 'checked' : '' }}>
                        <label class="form-check-label" for="abbattimento1">
                            Sì
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="abbattimento" id="abbattimento2" value="0" onchange="checkEmptiness()" {{ $campaigns->dropCall == 0 ? 'checked' : '' }}>
                        <label class="form-check-label" for="abbattimento2">
                            No
                        </label>
                    </div>
                    ...
                    <label id="enabledLabel">Attiva</label>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="enabled" id="enabled1" value="1" onchange="checkEmptiness()" {{ $campaigns->enabled == 1 ? 'checked' : '' }}>
                        <label class="form-check-label" for="enabled1">
                            Sì
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="enabled" id="enabled2" value="0" onchange="checkEmptiness()" {{ $campaigns->enabled == 0 ? 'checked' : '' }}>
                        <label class="form-check-label" for="enabled2">
                            No
                        </label>
                    </div>
                ...
                <div class="mb-3" id="startDateGroup">
                    <label>Data iniziale</label>
                    <input type="date" class="form-control" id="dataInizio" name="dataInizio" value="{{ \Carbon\Carbon::parse($campaigns->dateStart)->format('Y-m-d') }}">
                    <label style="margin-top: 10px;">Ora iniziale</label>
                    @php
                        $dateStart = \Carbon\Carbon::parse($campaigns->dateStart);
                        $selectedHour = $dateStart->format('H');
                        $selectedMinute = $dateStart->format('i');
                    @endphp
                    ...
                    <select name="startHour" style="border: 1px solid lightgray; background-color: white; border-radius: 10px;">
                        ...
                        @for ($i = 0; $i < 24; $i++)
                            @php
                                $value = $i;
                                $label = str_pad($i, 2, '0', STR_PAD_LEFT); // es. 01, 02...
                            @endphp
                            <option value="{{ $value }}" {{ $selectedHour == $label ? 'selected' : '' }}>{{ $label }}</option>
                        @endfor
                    </select>
                    : 
                    <select name="startMinute">
                        ...
                        @for ($i = 0; $i < 60; $i++)
                            ...
                            <option value="{{ $m }}" {{ $selectedMinute == $m ? 'selected' : '' }}>{{ $m }}</option>
                        @endfor
                    </select>
                </div>
                <div class="mb-3" id="endDateGroup">
                    <label>Data finale</label>
                    <input type="date" class="form-control" id="endDate" name="dataFine" value="{{ \Carbon\Carbon::parse($campaigns->dateEnd)->format('Y-m-d') }}">
                    <label style="margin-top: 10px;">Ora finale</label>
                    @php
                        $dateEnd = \Carbon\Carbon::parse($campaigns->dateEnd);
                        $selectedHour = $dateEnd->format('H');
                        $selectedMinute = $dateEnd->format('i');
                    @endphp
                ...
                <div class="mb-3" id="queueGroup">
                    ...
                    <select name="queue" id="queue" style="border: 1px solid lightgray; background-color: white; border-radius: 10px;">
                            <option value=""></option>
                            @foreach($queues as $queue)
                                <option value="{{ $queue->queue }}" {{ $campaigns->queue == $queue->queue ? 'selected' : '' }}>{{ $queue->queue }} | {{ $queue->name }}</option>
                            @endforeach
                    </select>
                </div>
                <div class="mb-3" id="forceQueueGroup">
                    ...
                    <select name="forzaCoda" id="forzaCoda" style="border: 1px solid lightgray; background-color: white; border-radius: 10px;">
                        <option value=""></option>
                            @foreach($queues as $queue)
                                <option value="{{ $queue->Queue }}" {{ $campaigns->toQueue == $queue->queue ? 'selected' : '' }}>{{ $queue->queue }} | {{ $queue->name }}</option>
                            @endforeach
                    </select>
                </div>
                <div class="mb-3" style="display: flex; flex-direction: row;">
